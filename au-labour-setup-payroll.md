...
 
  ...
  GENERAL FASTTAB – Transaction Type codes are used
...
To access the Payroll Posting Group, go to the
following menu:
...
 
  ...
  From the AssistButton, select the appropriate
  On Cost Transaction Type Code.
  ...
  Enter in a Start Date if this transaction is
  only for a specified period, otherwise it can be left blank.
  ...
  Enter in an End Date if this transaction is
  only for a specified period, otherwise it can be left blank.
  ...
  Enter in a Pay Rate.  The value should be expressed in decimal
  format.  E.g. if the value I 5%, then
  enter 0.05
  ...
  If this transaction is a Superannuation On Cost
  transaction, you must specify the Employee’s Superannuation Code from the
  AssistButton.
